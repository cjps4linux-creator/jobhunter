...
def test_ci_trigger_branches_are_master():
    data, triggers = _load_workflow()
    assert triggers.get("push", {}).get("branches") == ["master"]
    assert triggers.get("pull_request", {}).get("branches") == ["master"]
...
def test_ci_test_job_uses_pytest_with_coverage():
    data = yaml.safe_load(WORKFLOW.read_text(encoding="utf-8"))
    steps = " ".join(step.get("run", "") for step in data["jobs"]["test"]["steps"])
